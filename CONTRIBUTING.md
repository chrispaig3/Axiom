...
2026-09-03, `axiom fmt --check` over every one of the 594 `.ax` files
in the repository answers `is already formatted` for 477 of them and
`needs formatting` for 117. Two of the 117 are deliberate and are named
below; the other 115 were committed unformatted, and that same sweep is
...
| `build-shared-axc.sh` | not an assertion but the step the others rest on: it builds the compiler under test ONCE and stamps it, and the fifty-five gates that call `gate_build_axc` reuse it while the stamp matches the tree. It builds a second time and compares the IR both compilers emit, because "this artifact is what you would have built" is the claim fifty-five gates then rest on |
