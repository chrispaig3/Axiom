...
2026-09-21, `axiom fmt --check` over every one of the 692 `.ax` files
in the repository answers `is already formatted` for 687 of them and
`needs formatting` for 5. Four are deliberate and are named here:
...
way because its spans are pinned. The fifth is
`self_host/explain.ax`, which is drift rather than intent — it arrived
needing formatting with the AX3072 change and nobody has formatted
it since — and
that same sweep is
...
| `build-shared-axc.sh` | not an assertion but the step the others rest on: it builds the compiler under test ONCE and stamps it, and the seventy-eight gates that call `gate_build_axc` reuse it while the stamp matches the tree. It builds a second time and compares the IR both compilers emit, because "this artifact is what you would have built" is the claim seventy-eight gates then rest on |
