...
the 556 `.ax` files in the repository apart from the two named below,
...
| `check-restrictions.sh` | `;@axiom:restrict(...)` is a check and never a transformation: stripping every restriction tag from a program changes no emitted IR byte and no AXSYM row beyond `#restrict=`; a satisfied restriction is silent on every control; each of the four restrictions goes red when its violation is planted in a copy; and a compiler whose `checkRestricts` answers nothing fails the fixtures |
