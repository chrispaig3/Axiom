...
whose visible half holds the length and the byte address (the same
layout `Str.strWrap` builds at run time), preceded by the two
reference-count header words every heap block carries, with the
static's count pinned at the immortal `-1` sentinel:

```llvm
@str_0    = private unnamed_addr constant [14 x i8] c"Hello, Axiom!\00"
@strhdr_0 = private constant { i64, i64, i64, ptr } { i64 -1, i64 0, i64 13, ptr @str_0 }, align 16
```
...
| Editor support | **Functional** | [tree-sitter grammar](tree-sitter-axiom/) with highlighting queries, gated against all 339 `.ax` files in the repo and a 32-case tree-shape corpus. The language server is `self_host/lsp.ax`, listed four rows above and gated by `scripts/check-lsp-selfhost.sh` |
