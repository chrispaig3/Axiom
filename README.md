...
Twenty-one modules, all of them Axiom source under `stdlib/`. The
...
standard-library case for both under their own triples on every PR,
`check-platform-constants.sh` holds the emitted runtime's syscall
numbers to `stdlib/Sys/Platform.freebsd.ax`'s, and nothing has executed
the result - no seed, no artifact, no runner. FreeBSD 12 is the floor
...
axiom version
#   axiom (self-hosted) 0.3.7 (build 7ce43b921d1d 23b97d8285b4)
...
```
axiom (self-hosted) 0.3.7 - Axiom REPL
...
| Cross-compilation | **Functional** | `--target` selects ABI and platform stdlib modules; every stdlib case assembled from one host for all five targets at three levels (`scripts/check-cross-targets.sh`), executed in CI on three of them; `windows-x86_64` emits and is neither linked nor run anywhere yet (see *Targets*) |
