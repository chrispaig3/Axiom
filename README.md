...
| Region syntax | **Annotations landed; the form is reserved** | A signature may name the region a reference lives in, `(:: intern (-> (String @s) (Table @r) (Sym @r)))`, and the escape rule `MM-RGN-3` of [docs/memory-model-v2-design.md](docs/memory-model-v2-design.md) refuses a store, a return or a capture whose place outlives the value (`AX3060`–`AX3063`, `tests/diagnostics/640`–`644`, `scripts/check-region-escape.sh`; stage S3 of the note, 2026-09-03). An un-annotated program is untouched: the pass is absent when no signature names a region, and an annotated program emits byte-identical IR to its stripped twin. The `(region r ...)` form is stage S2 and still reports `AX2004`; allocating into a named region is S4 |
