...
A capability record's members are ordinary functions, so they declare
their own effects the way every other function does — there is no
member-tag rule to learn, which is most of the point of the record.
A function that performs I/O and says nothing draws `AX3042` at its own
name, and the effect travels through the record: `main` calls the field,
so `main` performs I/O too and must say so.
...
axiom version
#   Axiom 0.6.0 (build 7ce43b921d1d 23b97d8285b4)
