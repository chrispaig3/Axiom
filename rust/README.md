...
| `axiom-ffi-classify` | The ONE type table, attribute grammar and naming rule, shared by the macro and bindgen so the two passes over an annotation cannot drift. Closed: every accepted type is enumerated, anything else is refused with the list. |
| `axiom-ffi` | The facade a crate author depends on. `std` by default; `nostd-runtime` supplies the allocator, panic handler and memory intrinsics a `no_std` crate needs. Defines `axffi_free_bytes` and `axffi_abi_version` (ABI 2). |
| `axiom-ffi-macros` | `#[axiom_export]` — generates the `#[no_mangle] extern "C"` shim; `#[axiom_opaque]` — marks a handle type and generates its `axffi_<t>_drop` / `axffi_<t>_drop_fn` pair. UI-tested with `trybuild`. |
...
| `examples/demo` | `std`. Scalars, narrow ints, strings, bytes, `Result`, `Option`, opaque handles. |
...
&[u8] &T &mut T`. Returns: those, plus `() String Vec<u8> Option<T>
Result<T, E>` and an owned `T`. `T` must carry `#[axiom_opaque]`;
anything else (`u64`, `char`, `Vec<i64>`, a by-value `T`) is a compile
error that lists the set. Narrow integers are range-checked at the
...
cargo run -p axiom-bindgen -- --src examples/demo/src --lib axiom_demo \
  --check examples/demo/axiom/Demo.ax      # exit 1 when stale
```
