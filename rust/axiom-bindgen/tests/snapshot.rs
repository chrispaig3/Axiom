...
fn nested_fixture_shape() {
    ...
    assert!(fresh
        .contains("(pub :: combine (-> (-> Int Int Int) (-> Int Int Int Int) Int (Option Int)))"));
    ...
    assert!(
        fresh.contains("(pub :: piecesLookup (-> String (Option (Result (Vec String) String))))")
    );
    ...
    assert!(fresh.starts_with(
        "; GENERATED by axiom-bindgen from the `axiom_nested` crate. Do not edit.\n(import Ffi)\n"
    ));
}
...
fn camel_case_collision_is_an_error() {
    ...
    assert!(
        err.0.contains("`foo_bar`") && err.0.contains("`fooBar`"),
        "{err}"
    );
}
...
fn unmarked_opaque_is_an_error() {
    ...
    assert!(
        err.0
            .contains("`Plain` crosses the boundary as an opaque handle"),
        "{err}"
    );
...
fn unrecorded_by_value_is_an_error() {
    ...
    assert!(
        err.0
            .contains("`Plain` crosses as a record but no `#[axiom_record]` declaration"),
        "{err}"
    );
...
fn unrecorded_in_vec_is_an_error() {
    let err =
        axiom_bindgen::generate(&fixture("unrecorded_vec/src"), "axiom_unrecorded").unwrap_err();
    assert!(
        err.0
            .contains("`Plain` crosses as a record but no `#[axiom_record]` declaration"),
        "{err}"
