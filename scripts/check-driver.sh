...
# --- the Windows link ------------------------------------------------
#
# `build --target=windows-x86_64` links with `lld-link`, not `cc`, and
# every step of that path is asserted here on a host that cannot run
# the result: the executable is spelled `.exe` exactly once, the import
# library is found under the `.lib` spelling, a missing or failing
# `lld-link` is blamed by name (AX4003, the same way llc and cc are),
# an absent `kernel32.lib` is AX4004 naming the runtime's own import,
# and `--emit-staticlib` is refused rather than half-done. `kernel32.lib`
# is generated by `llvm-dlltool` from a `.def` written here - no Windows
# SDK is involved - which is also how the CI leg gets one.
for tool in lld-link llvm-dlltool; do
  if ! command -v "$tool" > /dev/null 2>&1; then
    bad "$tool is not on PATH; the Windows link cases below need LLVM's lld and llvm-dlltool"
  fi
done
if command -v lld-link > /dev/null 2>&1 && command -v llvm-dlltool > /dev/null 2>&1; then
  mkdir -p "$work/winlib"
  printf 'LIBRARY kernel32.dll\nEXPORTS\nVirtualAlloc\nGetStdHandle\nWriteFile\nExitProcess\nGetCommandLineW\nGetEnvironmentStringsW\nReadFile\nGetLastError\n' > "$work/winlib/kernel32.def"
  llvm-dlltool -m i386:x86-64 -d "$work/winlib/kernel32.def" -l "$work/winlib/kernel32.lib"

  "$s1" build --target=windows-x86_64 --input hello.ax --output whello --link-search "$work/winlib" >whello.log 2>&1; rc=$?
  if [[ $rc == 0 && -f whello.exe && ! -e whello ]] && [[ "$(head -c 2 whello.exe)" == "MZ" ]]; then
    ok "a windows-x86_64 build links a PE through lld-link and spells it whello.exe, once"
  else
    bad "windows-x86_64 build (rc=$rc; $(ls whello* 2>/dev/null | tr '\n' ' '))"
    sed 's/^/     /' whello.log | head -5
  fi
  [[ ! -f whello.o && ! -f whello.ll && ! -f whello.opt.ll && ! -f whello.exe.ll ]] \
    && ok "the Windows build removes its intermediates, and keeps the stem for them" \
    || bad "the Windows build left intermediates behind: $(ls whello* 2>/dev/null | tr '\n' ' ')"

  "$s1" build --target=windows-x86_64 --input hello.ax --output whello2.exe --link-search "$work/winlib" >/dev/null 2>&1
  [[ -f whello2.exe && ! -e whello2.exe.exe ]] \
    && ok "an output already spelled .exe is not spelled .exe.exe" \
    || bad "an output already spelled .exe: $(ls whello2* 2>/dev/null | tr '\n' ' ')"

  # The same build with `--emit-llvm` keeps `whello3.ll` beside the exe.
  "$s1" build --target=windows-x86_64 --input hello.ax --output whello3 --emit-llvm --link-search "$work/winlib" >/dev/null 2>&1
  [[ -f whello3.exe && -f whello3.ll ]] && grep -q 'x86_64-pc-windows-msvc' whello3.ll \
    && ok "--emit-llvm keeps whello3.ll beside whello3.exe, with the Windows triple" \
    || bad "--emit-llvm on a Windows build: $(ls whello3* 2>/dev/null | tr '\n' ' ')"

  # No kernel32.lib on any search directory: the grounding pass refuses
  # BEFORE the toolchain runs, naming the runtime's own import.
  "$s1" build --target=windows-x86_64 --input hello.ax --output wnolib >wnolib.log 2>&1; rc=$?
  if [[ $rc != 0 && ! -f wnolib.exe ]] && grep -q 'AX4004' wnolib.log && grep -q 'VirtualAlloc' wnolib.log; then
    ok "a Windows build with no kernel32.lib is refused as AX4004 naming VirtualAlloc, before any tool runs"
  else
    bad "a Windows build with no kernel32.lib (rc=$rc, first line: $(head -1 wnolib.log))"
  fi

  # A PATH with no lld-link on it: blamed by name, exit 4.
  mkdir -p fake-nolld
  for t in llc opt; do ln -sf "$(command -v $t)" "fake-nolld/$t"; done
  PATH="$work/fake-nolld" "$s1" build --target=windows-x86_64 --input hello.ax --output wnolld --link-search "$work/winlib" >wnolld.log 2>&1; rc=$?
  if [[ $rc == 4 && ! -f wnolld.exe ]] && grep -q 'could not run `lld-link`' wnolld.log && grep -q 'AX4003' wnolld.log; then
    ok "a missing lld-link fails the Windows build with exit 4 and names lld-link (AX4003)"
  else
    bad "a missing lld-link (rc=$rc, blamed: $(head -1 wnolld.log))"
  fi

  # A failing lld-link: also blamed by name, and with no .exe left.
  mkdir -p fake-lld
  printf '#!/bin/sh\nexit 1\n' >fake-lld/lld-link && chmod +x fake-lld/lld-link
  PATH="$work/fake-lld:$PATH" "$s1" build --target=windows-x86_64 --input hello.ax --output wbadlld --link-search "$work/winlib" >wbadlld.log 2>&1; rc=$?
  if [[ $rc == 4 && ! -f wbadlld.exe ]] && grep -q 'lld-link failed' wbadlld.log && grep -q 'AX4003' wbadlld.log; then
    ok "a failing lld-link fails the Windows build with exit 4, no executable, and names lld-link (AX4003)"
  else
    bad "a failing lld-link (rc=$rc, blamed: $(head -1 wbadlld.log))"
  fi

  # `--emit-staticlib` for Windows is refused, in words, with exit 4.
  "$s1" build --target=windows-x86_64 --input hello.ax --output wlib.a --emit-staticlib --link-search "$work/winlib" >wlib.log 2>&1; rc=$?
  if [[ $rc == 4 && ! -f wlib.a ]] && grep -q 'not supported for windows-x86_64' wlib.log && grep -q 'AX4003' wlib.log; then
    ok "--emit-staticlib for windows-x86_64 is refused in words (AX4003, exit 4), not half-done"
  else
  ...
  [[ -x hostbuild && ! -e hostbuild.exe ]] && [[ "$(./hostbuild)" == "driver-ok" ]] \
