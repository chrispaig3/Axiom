...
#             stdlib it reaches. 4,303 functions; 2,717 perform, 2,054
#             of those exactly `Alloc,Mut`; `Mut` anywhere in 2,543 of
#             the 2,717 (94%).
...
have "$(bucket "$work/main.axsym" 'Alloc,Mut')" 2054 "exactly Alloc,Mut"
...
have "$(grep '^F ' "$work/main.axsym" | grep -vc '#effects=\|#effects-incomplete' || true)" 1586 "pure (neither row nor mark)"
