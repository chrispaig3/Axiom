...
#      `tests/stdlib/467-region-signatures.ax` against its stripped
...
#
#   2. THE RULE IS LOAD-BEARING. The five fixtures 640-644 are refused
...
#      640-643. Then the program that compiler lets through is RUN:
...
#      turned into the thing this stage makes unspellable. 644's
...
cp "$repo_root/tests/stdlib/467-region-signatures.ax" "$work/run/ann.ax"
...
if ! "$axc" emit-llvm "$work/run/ann.ax" > "$work/run/ann.ll" 2> "$work/run/ann.err"; then
  ...
  if [[ "$lines" -lt 500 ]]; then
    ...
    ok "467 with and without its annotations: byte-identical IR ($lines lines)"
  fi
fi
# and it RUNS to its golden, which is what makes the identity worth having
checks=$((checks + 1))
if "$axc" run "$work/run/ann.ax" > "$work/run/ann.out" 2>&1 \
   && diff -q "$work/run/ann.out" "$repo_root/tests/stdlib/467-region-signatures.out" > /dev/null; then
  ok "467 runs to its golden under the annotations"
else
  bad "467 did not run to its golden"; sed 's/^/     /' "$work/run/ann.out" | head -10
fi
...
refused 640-region-escape-store   AX3060 'field `s`'
refused 640-region-escape-store   AX3060 'parameter `v` of `vecPush`'
refused 641-region-escape-return  AX3061 'pick'
refused 641-region-escape-return  AX3063 'orphan'
refused 642-region-capture        AX3062 'captures `s`'
refused 643-region-argument       AX3063 'pair'
refused 644-restrict-no-escape    AX3049 'through `vecPush`'
refused 644-restrict-no-escape    AX3051 'dispatch'
refused 644-restrict-no-escape    AX3057 'dispatchStrict'
...
if [[ $? -ne 0 ]]; then
  ...
  accepted 640-region-escape-store
  accepted 641-region-escape-return
  accepted 642-region-capture
  accepted 643-region-argument
  accepted escape-store
  # 644 rests on the facts, not the pass, and must survive
  checks=$((checks + 1))
  ( cd "$work/run" && AXIOM_STDLIB="$abl/stdlib" "$work/axc-ablated" --diagnostic-format=ai check 644-restrict-no-escape.ax ) > /dev/null 2> "$work/run/644.abl"
  if grep -q '^E AX3049 .*through `vecPush`' "$work/run/644.abl"; then
    ok "ablated: 644's restrict(no-escape) refusal survives - it reads the facts, not the pass"
  else
    bad "ablated: 644's AX3049 vanished with the pass, so no-escape rests on the pass after all"
  fi
