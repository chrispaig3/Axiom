...
# is the preamble nineteen gates share, and `gate_build_axc` is the
...
# one of those nineteen gates at once - and the failure would look like
...
echo "                was built from the tree as it stands, so nineteen gates"
