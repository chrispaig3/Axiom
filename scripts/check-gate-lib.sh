...
# is the preamble seventy-eight gates share, and `gate_build_axc` is the
...
# one of those seventy-eight gates at once - and the failure would look like
...
word_for() {
  case "$1" in
    ...
    50) echo fifty ;;         51) echo "fifty-one" ;;
    52) echo "fifty-two" ;;   53) echo "fifty-three" ;;
    54) echo "fifty-four" ;;
    55) echo "fifty-five" ;;
    56) echo "fifty-six" ;;
    57) echo "fifty-seven" ;;
    ...
    76) echo "seventy-six" ;;
    77) echo "seventy-seven" ;;
    78) echo "seventy-eight" ;;
    *)  echo "" ;;
  esac
}
...
#
# THE PAIR LIST BELOW IS HAND-WRITTEN AND HAD GONE STALE AGAIN. It
# stopped at fifty-six on 2026-09-03 while `word_for` answered up to
# sixty, so the four arms the count had most recently moved through
# were the four this guard could not see - the same shape the note
# further down records being closed twice already, in the file whose
# subject is a check whose domain excludes the cases it is for. It is
# extended here to the table's end because a new arm was added beside
# it; a guard that does not cover the arm just written is not a guard.
...
for pair in "15 fifteen" "16 sixteen" "17 seventeen" "18 eighteen" \
            ...
            "71 seventy-one" "72 seventy-two" "73 seventy-three" "74 seventy-four" "75 seventy-five" "76 seventy-six" "77 seventy-seven" "78 seventy-eight"; do
  set -- $pair
  arms=$((arms + 1))
  got="$(word_for "$1")"
  if [[ "$got" != "$2" ]]; then
...
echo "                was built from the tree as it stands, so seventy-eight gates"
