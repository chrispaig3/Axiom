...
# is the preamble fifty-eight gates share, and `gate_build_axc` is the
...
# one of those fifty-eight gates at once - and the failure would look like
...
for pair in "15 fifteen" "16 sixteen" "17 seventeen" "18 eighteen" \
            ...
            "53 fifty-three" "54 fifty-four" "55 fifty-five" "56 fifty-six"; do
  set -- $pair
  arms=$((arms + 1))
  got="$(word_for "$1")"
  if [[ "$got" != "$2" ]]; then
...
echo "                was built from the tree as it stands, so fifty-eight gates"
