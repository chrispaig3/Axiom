...
# is the preamble forty gates share, and `gate_build_axc` is the
...
# one of those forty gates at once - and the failure would look like
...
for pair in "15 fifteen" "16 sixteen" "17 seventeen" "18 eighteen" \
            ...
            "39 thirty-nine" "40 forty" "41 forty-one" "42 forty-two"; do
  set -- $pair
  got="$(word_for "$1")"
  if [[ "$got" != "$2" ]]; then
...
echo "                was built from the tree as it stands, so forty gates"
