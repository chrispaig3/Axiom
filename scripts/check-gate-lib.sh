...
# is the preamble sixty-two gates share, and `gate_build_axc` is the
...
# one of those sixty-two gates at once - and the failure would look like
...
echo "                was built from the tree as it stands, so sixty-two gates"
