...
# is the preamble forty-one gates share, and `gate_build_axc` is the
...
# one of those forty-one gates at once - and the failure would look like
...
echo "                was built from the tree as it stands, so forty-one gates"
