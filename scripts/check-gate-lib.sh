...
# is the preamble seventy-one gates share, and `gate_build_axc` is the
...
# one of those seventy-one gates at once - and the failure would look like
...
echo "                was built from the tree as it stands, so seventy-one gates"
