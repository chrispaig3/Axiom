...
# is the preamble sixty-six gates share, and `gate_build_axc` is the
...
# one of those sixty-six gates at once - and the failure would look like
...
echo "                was built from the tree as it stands, so sixty-six gates"
echo "                was built from the tree as it stands, so sixty-six gates"
