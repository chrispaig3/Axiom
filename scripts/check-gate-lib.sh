...
# is the preamble fifty-one gates share, and `gate_build_axc` is the
...
# one of those fifty-one gates at once - and the failure would look like
...
for pair in "15 fifteen" "16 sixteen" "17 seventeen" "18 eighteen" \
            ...
            "49 forty-nine" "50 fifty" "51 fifty-one"; do
  set -- $pair
  arms=$((arms + 1))
  got="$(word_for "$1")"
  if [[ "$got" != "$2" ]]; then
...
echo "                was built from the tree as it stands, so fifty-one gates"
