...
# is the preamble seventy-seven gates share, and `gate_build_axc` is the
...
# one of those seventy-seven gates at once - and the failure would look like
...
for pair in "15 fifteen" "16 sixteen" "17 seventeen" "18 eighteen" \
            ...
            "71 seventy-one" "72 seventy-two" "73 seventy-three" "74 seventy-four" "75 seventy-five" "76 seventy-six" "77 seventy-seven"; do
  set -- $pair
  arms=$((arms + 1))
  got="$(word_for "$1")"
  if [[ "$got" != "$2" ]]; then
...
echo "                was built from the tree as it stands, so seventy-seven gates"
