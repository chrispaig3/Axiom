...
# is the preamble thirty-three gates share, and `gate_build_axc` is the
...
# one of those thirty-three gates at once - and the failure would look like
...
for pair in "15 fifteen" "16 sixteen" "17 seventeen" "18 eighteen" \
            ...
            "32 thirty-two" "33 thirty-three" "34 thirty-four"; do
  set -- $pair
  got="$(word_for "$1")"
  if [[ "$got" != "$2" ]]; then
...
if (( table_ok )); then
  echo "ok   word_for answers its own 20 arms"
else
  failed=$((failed + 1))
fi
want="$(word_for "$n_axc")"
...
echo "                was built from the tree as it stands, so thirty-three gates"
