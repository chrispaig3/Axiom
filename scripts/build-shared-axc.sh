...
# trust it.
#
# sixty-one gates call `gate_build_axc`, and each one rebuilt the same
...
# the claim sixty-one gates then rest on - "this artifact is what you
...
# sixty-one gates is the compiler each of them would have built exactly
...
# emitted byte is not a difference the sixty-one gates can observe, and
...
$(cmp "$work/a.ll" "$work/b.ll" 2>&1 | sed 's/.*line //')" >&2
  ...
  echo "      have built, and sixty-one gates would be testing something else." >&2
  exit 1
fi
