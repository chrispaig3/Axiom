...
# trust it.
#
# Thirty-eight gates call `gate_build_axc`, and each one rebuilt the same
...
# the claim thirty-eight gates then rest on - "this artifact is what you
...
# thirty-eight gates is the compiler each of them would have built exactly
...
# emitted byte is not a difference the thirty-eight gates can observe, and
...
$(cmp "$work/a.ll" "$work/b.ll" 2>&1 | sed 's/.*line //')" >&2
  ...
  echo "      have built, and thirty-eight gates would be testing something else." >&2
  exit 1
fi
