...
function Fixpoint() {
  const stops = [
    ...
    { x: 2, label: 'stage2', sub: 'built by the seed' },
    { x: 3, label: 'stage3', sub: 'built by stage2' },
  ]
  ...
  return (
    <div className="fixpoint">
      <svg
        viewBox={`0 0 ${W} ${H}`}
        role="img"
        aria-label="bootstrap slash star dot l l, run through l l c and c c, produces a seed compiler; the seed compiles self_host into stage 2; stage 2 compiles self_host into stage 3; stage 2 and stage 3 must be byte-identical."
      >
        {/* the chain */}
        ...
        {stops.map((s, i) => (
          <g key={s.label}>
            <circle
              className={i >= 2 ? 'fixpoint__dot fixpoint__dot--key' : 'fixpoint__dot'}
              cx={pad + s.x * step}
              cy={y}
              r={i >= 2 ? 6 : 4.5}
            />
        ...
        <path
          ...
          d={`M${pad + 2 * step} ${y + 40} v14 h${step} v-14`}
        />
        <text
