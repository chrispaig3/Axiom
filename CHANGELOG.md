...
`__store64`, `__alloc`, `__addr` — infer the `Unsafe` effect now, the
sixth built-in after `IO`, `Pure`, `Alloc`, `Mut` and `Div`: ambiently
and transitively like `Alloc` and `Mut`, checked against
`;@axiom:pure` and `restrict(no-unsafe)`. And the declaration is
required: a function whose own body calls one of the six without
`;@axiom:effect(unsafe)` is the new `AX3073 undeclared-unsafe`, an
error at the first call in source order, reported once per
declaration. Lexical, not transitive like `AX3042` — a transitive
rule would tag 3,910 of the 4,021 effectful functions in this tree,
so the obligation sits where the primitive is called and wrappers
contain it (48 tags across `self_host/` and `stdlib/`).
Unannotated unsafe code fails to compile. Held by
`scripts/check-diagnostics.sh` (1004 is the primary, and the silence
sweep over the compiler's own source), the re-pin in
`scripts/check-effect-distribution.sh` (2175 to 2181 in
`Alloc,Mut,Unsafe`, every other bucket frozen), and
`scripts/check-doc-drift.sh` in both directions on the new code.
...
hundred of them element and attribute macros, the seventeen `h*`
builder and escaper functions underneath, and the `HtmlBuf` struct.
The compiler did not use it and no other library module imported it.
Measured at `8f9c6c8`, its importers were the two fixtures that pinned
it and `examples/web/server.ax`, the templated server that was its one
program. A library module with one program is that program's private
helper wearing a public surface, and every one of those 118 names was
a row in `docs/stdlib-api.md`, a line in `tests/agent/stdlib-effects.allow`
and a contract in `compat/`. All of it is gone: the module, its two
fixtures, the example (`server.ax`, `static/app.js`, `static/site.css`)
and the web half of the gate that served it.

**What a program that imported `Html` should do.** The module never
depended on anything a program cannot also import — `Mem`, `Str`,
`Vec` — so it compiles unchanged as a file beside the entry file
(measured: `git show 8f9c6c8:stdlib/Html.ax` copied next to a
program that `(import Html)`s and renders `(div b { (text b ...) })`
checks and runs under the compiler built from this tree; module
resolution reads the entry file's own directory first). Nothing in the
library replaces it,
and nothing will: writing bytes through `Str` is what it did.

**The gate is renamed, not deleted, because it was two gates in one
file.** `scripts/check-web.sh`'s subject — build the server, compare
every page byte for byte against a response written by hand, bypass
the escaper and demand the raw `<script>` back in both positions,
compile the path check out and demand the planted file served, hold
worker RSS flat across ten thousand page requests (106x against the
same binary unscoped, floor 20x) — went with the example, and `git log
-- scripts/check-web.sh` keeps it. Its other half, the sweep of
`examples/` it carried only because it was the one thing CI pointed at
that directory, is `scripts/check-examples.sh`: every tracked file a
source or a static asset and none executable, every program named in
`examples/README.md` and `README.md` linking there, with the five
ablations it had and four more run by hand against the real paths
before it landed (a planted 100755 index row, a table row dropped, a
row naming a program that is gone, README's link removed — each red).
...
What else moved, each re-derived rather than edited: `docs/stdlib-api.md`
regenerated by its gate, 118 rows fewer, with `check-stdlib-api.sh`'s
name floor 700 → 595 and its documentation ratchet 560 → 445 set at the
measurement with the date (every one of the 118 rows carried a summary,
so the documented count fell by exactly that and the undocumented count
by nothing); `tests/agent/stdlib-effects.allow` re-derived by
`check-agent-policy.sh`, seventeen lines fewer (sixteen of the public
functions — `hLen` derives no effect and was never listed — and the
private `hEscape`); the tracked `.ax` census down by
four at its three sites (`CONTRIBUTING.md`, `docs/status.md`,
`web/src/data/site.ts`); the module lists in `docs/reference.md`,
`docs/status.md` and `CONTRIBUTING.md`; `docs/macro-system.md` §10.6,
which now records what the DSL established about the macro rules in
the past tense — including the one unnumbered rule only it exercised,
that a private *function* named by a template is refused at the
invocation with `AX3023`; and one sentence each in `docs/reference.md`,
`docs/generics-design.md`, `self_host/parser.ax`, `stdlib/Http.ax`,
`scripts/check-parallel.sh`, `tests/stdlib/466-for-loop.ax` and
`.gitignore` that described the module, the example or the gate as
present. `tests/fmt/corpus-fmt.golden` is not regenerated: its header
says a deleted file retires its entry and a regeneration is deliberate,
never automatic, and the gate's floors are nowhere near — measured, 600
of 627 pinned mappings still reproduce and 26 files are unpinned,
against a ceiling of 60.

**Two numbers this deletion moved that a first pass moved only half
of.** `CONTRIBUTING.md`'s formatter paragraph states one count and the
two numbers under it — 636 files, `is already formatted` for 411 and
`needs formatting` for 225 — and only the total was re-derived, leaving
a sentence whose own two halves summed to a tree that no longer
existed. Re-measured over all 632 with the compiler this tree builds:
**410 and 222**. The four deleted files account for the difference
exactly, three of them unformatted and `tests/stdlib/421-html-script.ax`
the one that was not, which is what makes 411/225 → 410/222 an
accounting rather than an adjustment. And `docs/status.md`'s Standard
library row, edited here from twenty-two names to twenty, was wrong in
three ways of which only one was this deletion: it had kept `Show`
since `stdlib/Show.ax` was deleted at 0.7.4, and it had never named
`Tui.Keys`, `Tui.Edit` or `Tui.Term` at all. It reads **twenty-three**
now, which is the count `docs/reference.md` states and the list it
prints.

Neither prose module list had a gate, which is why both rotted quietly
for two releases; `check-stdlib-api.sh` already owns the authoritative
list and now holds both to it — every name, both directions, and the
spelled-out count derived from the same list rather than typed beside
it, since the status row's "Twenty" MATCHED its own twenty names while
three modules were missing from both. Three negative probes: a module
dropped from a copy of each document, each asserting its edit landed
before believing the result, and the word table answering for itself at
every arm. `check-stdlib-api.sh` goes 9 checks → 23.

### Compatibility

- **Breaking.** `stdlib/Html.ax` and its 118 public names are removed:
  `HtmlBuf`, the seventeen `h*` functions and one hundred macros, each
  declared in `compat/BREAKING` against 0.7.6 (`for`, `forInt`,
  `hVecLen`, `hVecStr` and `hVecWord` were already declared at 0.6.4).
  `HtmlBuf` is declared even though the permit would have accepted its
  removal on the strength of the 0.6.4 retype row, because a ledger
  whose last word on a struct is "one field retyped" when the struct is
  gone misreads. A program that imported `Html` copies the module out
  of history beside its entry file, where it compiles unchanged.
