...
# generates `tagShape`, which `(main)` calls. Requests 26-29 target it
# and pin the MAC-TOOL-3 shape - definition, declaration and hover
# answer from the cache the fill wrote at `didOpen`, references sees
# the invocation first and the use after it. scripts/check-macro-demand.sh
# counts these four as the gated generated-name demand.
...
SIG_TY = cut(FIX, "(pub :: (syntax/join tag T) ", ")")  # the :: template's type
GEN_HOVER_CODE = "(pub :: " + GEN_NAME + " " + SIG_TY + ")"  # the cached rendering
GEN_HOVER_FORM = "(" + MACRO_NAME + " " + INV_ARG + ")"      # the invocation form
BROKEN_FIX = FIX + "\n("
FIX_LINES = FIX.split("\n")
...
fix_session = b"".join(frame(m) for m in [
    ...
    fixreq(29, "textDocument/declaration", fixdoc(fix_uri, {"position": at(TAGSHP_USE)})),
    {"jsonrpc": "2.0", "method": "textDocument/didOpen",
...
# `tagShape` exists only after expansion, so the raw-tree requests miss
# it - and the cache answers where they miss. The fill runs where the
# pipeline already runs, at `didOpen`/`didChange`, so these four read a
# string and expand nothing, which is what keeps MAC-TOOL-3 standing.
# These four are the gated demand count: if the cache ever stops
# answering them, the decision they pin is the thing to revisit, and
# scripts/check-macro-demand.sh trips on the count moving.
elif fres(26) != {"uri": fix_uri, "range": {"start": {"line": DERIVE_USE["line"], "character": DERIVE_USE["start"]}, "end": {"line": DERIVE_USE["line"], "character": DERIVE_USE["end"]}}}:
    fwhy = f"definition on generated `tagShape` answered {fres(26)!r:.200}, want the invocation head (MAC-TOOL-3-demand)"
elif fres(27) != {"contents": {"kind": "markdown", "value": "```axiom\n" + GEN_HOVER_CODE + "\n```\n\ngenerated by `" + GEN_HOVER_FORM + "`\n"}, "range": {"start": {"line": TAGSHP_USE["line"], "character": TAGSHP_USE["start"]}, "end": {"line": TAGSHP_USE["line"], "character": TAGSHP_USE["end"]}}}:
    fwhy = f"hover on generated `tagShape` answered {fres(27)!r:.200}, want the cached rendering (MAC-TOOL-3-demand)"
# References is the honest third: the invocation precedes the use in
# the document, so the cache prepends it in document order. Pin
# exactly that shape - an answer that loses the invocation is the
# fill going blind, and one that loses the use is the walk going
# blind.
elif (fres(28) or []) != [{"uri": fix_uri, "range": {"start": {"line": DERIVE_USE["line"], "character": DERIVE_USE["start"]}, "end": {"line": DERIVE_USE["line"], "character": DERIVE_USE["end"]}}}, {"uri": fix_uri, "range": {"start": {"line": TAGSHP_USE["line"], "character": TAGSHP_USE["start"]}, "end": {"line": TAGSHP_USE["line"], "character": TAGSHP_USE["end"]}}}]:
    fwhy = f"references on generated `tagShape` answered {fres(28)!r:.200}, want the invocation then the use (MAC-TOOL-3-demand)"
elif fres(29) != {"uri": fix_uri, "range": {"start": {"line": DERIVE_USE["line"], "character": DERIVE_USE["start"]}, "end": {"line": DERIVE_USE["line"], "character": DERIVE_USE["end"]}}}:
    fwhy = f"declaration on generated `tagShape` answered {fres(29)!r:.200}, want the invocation head (MAC-TOOL-3-demand)"
