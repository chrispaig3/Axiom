...
| `HttpHandler` | data |  |  | A handler: a function of the socket and the request, answering an Int the dispatcher passes back. Held in a cell because the router keeps handlers in a `Vec` of words. Written as `(HttpFn (lambda (fd r) (page fd r)))` around a signed `fn`. |
