...
(`MM-ALLOC-16b`). A violated contract gets **80**.

**It was designed on 75, it moved three times, and the second move
landed on an occupied number.** `MM-ALLOC-16a`'s trap and this one were
...
`91f33f7` had given `__indexTrap` on trunk in the meantime — and on
2026-09-08 it moved to **80**, the first free number, because two
broken invariants sharing one status is the defect `docs/ffi.md` §5.1
refused for 73, and `__indexTrap` held 77 on trunk first (roadmap item
11, D3; `MM-EXEC-16` carries the 80 row).

**77 was shared, and this note's own argument indicted it.** The whole
reason for taking 73 was that two failures at one status cannot be told
apart; that was true of a violated contract and an out-of-range
index, which differed only in the sentence on fd 2. Corrected
...
number that means `MM-ALLOC-16b` — and decided 2026-09-08, when the
contract trap moved to 80 (D3).
...
The same file without that first statement exits 80. The `post` arm was
...
0)`), both exit 80, because the compare is inside `half` and not at
either call.
...
| Runtime | `@__axiom_contract_fail`, emitted unconditionally and pruned where nothing calls it, exit **80** — its own row in `MM-EXEC-16` since D3 |
