> **Superseded design record — not a contract.** This is `docs/ffi.md` as it
> stood on 2026-08-22 before the FFI matured, moved here unchanged. It is six
> successive drafts written over one month, each revising the last, all under a
> "Status: complete" banner that was true of §0–§3 and of nothing after them.
> The normative description of what ships is [`../ffi.md`](../ffi.md).
> What shipped from these drafts: the `extern` block with inline types and the
> `(symbol "...")` clause; `Foreign` as a builtin kept out of the reference map;
> the out-cell + status-word protocol for bytes and `Result` (now also `Option`);
> the three measured tiers (`nm -u` 0 / 0 / 188) and the allowlist gate with its
> negative probes; ABI versioning (now 2). What did NOT ship, and is described
> below as if it had: `(pub export axiom ...)` and every Rust→Axiom direction,
> `ffi.manifest.json` / `ffi.lock` / `--ffi` / `--staticlib`, `__axiom_abi_guard`,
> a distinct `FFI` effect, `(opaque T (drop f))` and AX3037–AX3046, `Slice` and
> `Outcome` as types, `axiom_rt_init`. The handle protocol that replaced the
> `opaque` clause is `Handle` + `#[axiom_opaque]` (see `../ffi.md` §6).
