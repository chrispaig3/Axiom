...
**A STATED HOLE, measured 2026-08-30: inside a LIFTED LAMBDA the stamp
is the constant `0` whatever the argument's type.** `emitLamDef`
emits `define i64 @_lam_N(i64 %.env, ...)` with no evidence parameter,
so `emitEvwRead` falls back to its constant — "not a reference" — for a
parameter that is one, and the share is not taken. One program holding
the same `vecPush s` twice shows both readings, the trailing operand
being the evidence word:

    in a named `fn`   call @Vec$vecPush(i64 %box, i64 %s, i64 1)
    in `_lam_0`       call @Vec$vecPush(i64 %.t2, i64 %m, i64 0)

This is not observable today, and the reason is worth stating because
it is load-bearing rather than lucky: nothing releases a closure's
argument, so a block parked uncounted inside a lambda is kept alive by
the very leak `MM-LIFE-2c`'s event-5b note calls open. The two cancel.
Closing that leak without first giving a lifted lambda an evidence word
turns 96 bytes per operation into a use-after-free, which
`tests/stdlib/460-closure-reclaim.ax` term 64 exists to refuse. That last case is why this is
...
| `MM-LIFE-2j` | **a trait DEFAULT body's shape word depends on `impl` declaration order.** `checkImplComplete` synthesizes the default into every impl that omits the method **without copying the body's nodes**, so one AST is checked once per implementing type and per-node stamps are last-write-wins across the monomorphizations. Measured 2026-08-25 against the compiler as shipped at 0.3.0, on `tests/stdlib/373-shared-default-binder.ax` and the same file with its two `impl` blocks swapped: `Ident#String#ident` builds its block with header `131076` and `axiom_retain(%x)` when the `Int` impl is declared first, and with header **`4`** — a LEAF — and no retain when the `String` impl is. One program, two orderings, two ownership shapes for the same monomorphization, so one of the two is wrong for `String`. What is NOT established is which, or any wrong runtime behaviour: both spellings answer correctly, because the field's other owner is the caller. Recorded rather than fixed, and recorded with the ordering because that is the part a reader would not predict. It is the same root cause as `ERR-MEM-4`'s stamp, which is why that stamp refuses to take a last answer (`docs/error-model.md` ERR-MEM-4, `scripts/check-fallible-reclaim.sh`) |
