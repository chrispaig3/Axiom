...
The two edges worth arguing about, because they are the ones that look
optional:
...
| `fmt` round-trips every file | §2.3; 390/390, gated by `scripts/check-fmt.sh`. This row read 190/190 until 2026-08-16 — true when it was written and never re-derived while the corpus doubled |
...
(**that prerequisite was discharged 2026-08-15**: the fiat is
deleted, and where a type variable still hides pointerhood the
evidence word answers it at run time — MM-ALLOC-20, MM-LIFE-2d);
...
copy-at-boundary made real today; the automatic half is now decided to
be reference counting rather than inference
([memory-model.md](memory-model.md) MM-LIFE-2a), and it has a measured
target to hit.
...
[self-hosting.md §2](self-hosting.md#2-capability-gaps-measured) no
longer all hold — this sentence said they did until 2026-08-16, and it
sat there while the bullets directly beneath it were marked **(DONE)**
one at a time, which is the shape of every stale claim in this
document. `B1`, `B4` and `S5` are `(RESOLVED)` in that document,
`B3`'s `Map` criterion is met at 1.80×, and `S1` is closed for nullary
constructors and open for constructors with fields. What still holds
is `B2`'s recursion bound and `S2`–`S4`. The ones on the critical path
here:
...
> **Superseded (2026-08-14).** The automatic-reclamation strategy is
> decided in the normative specification,
> [docs/memory-model.md](memory-model.md): **reference counting**
> (MM-LIFE-2a–2f) — counts on heap blocks, compiler-emitted
> retain/release, deterministic reclamation; cycles leak and that is
> the accepted, stated cost, with a cycle collector a later separable
> decision. The sketch below is kept as the record of the alternative.
> What survives of it unchanged: the explicit
> `__axiom_arena_mark`/`__axiom_arena_reset`/`__axiom_arena_reset_keeping`
> primitives (shipped and gated — §2.2), and the pointerhood
> prerequisite (MM-ALLOC-20), which reference counting needs
> identically and whose measured progress the spec quotes. The closing
> paragraph's premise — "cycles are not constructible" — was measured
> false (MM-LIFE-3), and losing that premise is part of why the
> decision moved.
...
reclaims a value whose lifetime genuinely outlives every arena. This
paragraph originally justified the first with "Axiom's data is immutable
and inductive, so cycles are not constructible", and both halves of that
are **measured false**: `set` mutates struct fields through every alias,
and heap cycles are constructible by two routes, one using nothing but
`stdlib/Vec` ([docs/memory-model.md](memory-model.md) MM-LIFE-3, MM-MUT-2).
The chosen reference counting states the cycle leak as an accepted cost
instead of assuming it away; a value that outlives everything still lives
until process exit, which remains the correct answer for a compiler
process.
...
> (single-rule substitution, renaming hygiene, qualified macro names,
> expansion backtraces, budgets, declaration macros v1 (rule-form
> `fn`/`::` generation from the entry file, 2026-08-14), pattern-based
> multi-rule selection and ellipsis repetition (2026-08-16), eleven
> `AX30xx` macro codes; status **Partial**) and what remains
> normative-but-unbuilt (literal-identifier patterns, which need scope
> sets, and rules over EXPRESSION templates). This parenthesis named
> multi-rule patterns and repetition as the outstanding work until
> 2026-08-16, when both came off the list: rules landed 2026-08-15,
> selection became a match in rule order the next day, and ellipsis
> repetition the same day — `tests/selfhost/390-multi-rule-macro.ax`
> 51, `392-macro-patterns.ax` 127, `393-macro-ellipsis.ax` 63. The
...
| | Status |
|---|---|
| A `derive`-style macro generates a working `Eq` instance for a `data` type, with exhaustiveness still checked on the generated `match` | **MET for sums (2026-08-14, same day, second commit).** [macro-system.md §10.2](macro-system.md)'s nullary `deriveEq` — `syntax/join` naming the generated declarations, `syntax/constructors` answered from the declaration list, `syntax/for` splicing one arm per constructor — runs **verbatim** as `tests/selfhost/374-derive-eq.ax` (101) and through every frontend consumer as `tests/frontend/070-derive-macro.ax` (42). Exhaustiveness is checked on the generated `match` — by construction it covers every constructor the type has *at expansion time*. The lens set landed the same day (`fields`, `same`, declaration- and argument-position `for` — §10.3's `deriveLenses` verbatim, `tests/selfhost/375-derive-lenses.ax`, 34), and the fieldful rung one commit later (`binders`/`fold` — the free-function `deriveEq` over a mixed-arity sum, `377-derive-eq-fieldful.ax`, 30, with the nullary case falling out of the empty fold). The `impl`-generating form landed next (`378-derive-eq-impl.ax`, 30 — derived instances COMPOSE: `(deriveEq Inner)` then `(deriveEq Outer)` dispatches the inner field's comparison to the first instance by its static type), and the `deriving` clause is REFUSED (`AX2004` naming the replacement; the formatter refuses with the parser — MAC-CAP-9, blast radius exactly the one zoo pair the spec counted). **The criterion is MET, and the library SHIPS**: `stdlib/Pre.ax` carries `deriveEq`, invoked like any prelude macro over entry-file and imported types alike (`379-derive-imported.ax`, 30; a private subject refuses at the invocation, `tests/diagnostics/550-derive-private-type.ax`) |
| A macro that introduces a binding named `x` does not capture a user's `x`, and the reverse | **MET in both directions (2026-08-16).** This cell read "Half, and the halves are not symmetric" until then. The forward direction is done for every binder a template can introduce — `let`, `let mut`, `lambda`, `match` arm patterns — by renaming to `<name>.<counter>`, gated by `tests/selfhost/361-macro-hygiene.ax` (143; the unfixed compiler answers 208). The reverse direction is **also done for a macro defined in a module** (`tests/selfhost/364-macro-definition-site.ax`, 157 against the unfixed compiler's `AX3004`): a template's free identifier now resolves where the macro was written, which closed a live wrong-answer bug — one macro in one module used to mean two different things depending on the caller's module, 6 against 50, with no diagnostic from either compiler because both were resolving a name that really was in scope. It reaches a macro defined in the ENTRY file too, since 2026-08-16. Entry-file declarations are still left bare, so there is still no distinct name to resolve to — and a template does not need one: a macro is a top-level declaration, so its template's free identifiers had no enclosing local scope where they were written, and one bit on the reference tells both resolvers to skip the local scope (`tests/selfhost/394-macro-entry-capture.ax`, 130). This sentence called that residue "a loud `AX3004` rather than a wrong answer" until 2026-08-15, when it was measured and found to be a SILENT wrong answer — 0 where the macro's own function answers 40 — and the `AX3004` it quoted came from an arity mismatch in the probe rather than from the capture. The refusal that made it loud, `AX3032`, is itself retired: it would now reject correct programs. The macro NAME itself is qualifiable since 2026-08-14 — `Pre::when` expands, and a qualified private macro is `AX3023` (this cell said "`Pre::when` is `AX3001`" until then). See [macros.md §3](macros.md) and the spec, [macro-system.md](macro-system.md) §3 |
| A type error inside an expansion reports the macro *and* the invocation site, both with real spans | **Done** (2026-08-14). The invocation is the primary span, in the file being compiled, and each enclosing macro is a frame naming it with its declaration's span in its OWN file — the `trace` element widened to `(name, span, unit)`, the AXDL `&` field gained a location, and `tests/diagnostics/490-expansion-backtrace.ax` pins one frame and a nested two. The data-model limit this cell used to record (a `Diag` carries one unit, `Vec of Str` frames) was resolved by widening the FRAME, not the Diag: the primary stays one-unit, and the frame carries the second. See [macro-system.md](macro-system.md) MAC-DIAG-4. The second-file snippet (MAC-DIAG-5) landed 2026-08-15: each frame now opens its macro's own file with the template line quoted under a caret, and the gate derives that block from the AXDL rather than trusting the golden |
...
rather than by discipline. All process-wide mutable globals — the seven
(five allocator words plus `argc`/`argv`), and the per-effect evidence
slots besides ([memory-model.md](memory-model.md) MM-PAR-3; this sentence
used to count the slots among the seven) — are private after
...
**Rationale.** Isolation comes from the process boundary, not from the
memory model: mutable aliasing *is* constructible inside one process
(`set` on a struct field is visible through every alias —
[memory-model.md](memory-model.md) MM-MUT-2, correcting this sentence,
which used to claim the opposite), and nothing the language or the
...
**Design guidance (for a third-party library).** (Written before the
reference-counting decision; under `Job` today each task is a process
with its own heap, which delivers the isolation below for free, and
under ARC "reclaimed at join" is counts, not arenas —
[memory-model.md](memory-model.md) MM-LIFE-2a, MM-PAR-6.)
...
after it.** This section used to schedule one; it now records why there
is nothing to schedule, and there never was anything to delete — no
`Http` module, no socket surface, no `Net` anything has ever existed in
`stdlib/`. `grep -ril http` over the repository answers with two clone
URLs, a CI badge, and this roadmap. The removal is the removal of an
*intention*.

**Why it does not belong in the core.** The standard library's own
admission test is the one every module in `stdlib/` already passes: it
is either something the compiler itself needs (`Str`, `Vec`, `Map`,
`Intern`, `Json`, `Rpc`, `Job`, `Sys`), or something no program can
write for itself because it needs a primitive only the compiler emits
(`Mem`, `IO`). HTTP is neither. The compiler does not speak it, and a
user can write all of it in ordinary Axiom the moment the sockets
exist. Shipping it in the core buys nothing and costs the three things
this project is most careful about:

- **A version-pinned protocol in an unversioned tree.** HTTP/1.1 has
  errata, HTTP/2 has a different framing layer, and TLS has a
  vulnerability schedule. `stdlib/` ships with the compiler and is
  bound to its release cadence; a security fix to a header parser
  should not need a compiler release.
- **A gate surface out of proportion to the language.** Every stdlib
  module is carried by `check-stdlib-selfhost.sh` (53 of 53 agreeing
  across both compilers, 2026-08-15) and `check-freestanding.sh`. A server has to
  be gated against *load*, which is a benchmark harness, a network
  namespace, and a flake budget — a category of gate the repository
  does not have and should not acquire to serve one module.
- **A conformance claim the project cannot keep.** "Axiom has HTTP in
  the box" is read as "Axiom's HTTP is correct", and the RFC surface is
  larger than the language's.

**What the core owes an external package instead.** The dependency the
deleted plan actually identified was never HTTP — it was
*non-blocking I/O*, which is core surface, because syscalls are. That
part stays, and it is the whole of what v1 should provide:

| Layer | Where it belongs | Status |
|---|---|---|
| `epoll`/`kqueue`, `socket`/`bind`/`listen`/`accept`, `O_NONBLOCK` | `stdlib/Sys` + a per-platform module, the pattern `stdlib/Sys/Platform.*.ax` already establishes | Not built; the only piece with a legitimate claim on core |
| Event loop, timers, readiness queue | user space, over the above | Writable today apart from the syscalls |
| Connection state, header map, router, body codec | an external package | `Vec`/`Map`/`Str`/`Utf8`/`Json` are already sufficient |
| TLS | an external package, and preferably a binding to a reviewed implementation — which Axiom cannot express, because there is no FFI | Not a v1 subject |

**The external-package pattern, concretely.** Axiom has no package
manager, and v1 does not add one; what it has is module resolution
(§P5) — `AXIOM_PATH`, then `AXIOM_STDLIB`, then exe-relative. That is
already enough to consume a third-party library as a checked-out
directory:

```bash
git clone https://example.invalid/axiom-http vendor/axiom-http
AXIOM_PATH=vendor/axiom-http axiom --diagnostic-format=ai build \
  --input src/main.ax --output server
```

```scheme
; vendor/axiom-http/Http/Server.ax  — a package module, not stdlib
(import Http.Server (serve))
(import Http.Request (reqPath))
```

A package is a directory of `.ax` files with dotted module paths, and
it is checked, formatted, and gated by exactly the same tools as this
repository's own code — `axiom check`, `axiom fmt --check`,
`axiom symbols`. Nothing about being outside `stdlib/` makes a module
second-class: `Http.Server` and `Str` differ in who ships them and on
what cadence, and in nothing else. The one thing a package cannot do
is reach a syscall the compiler does not emit, which is precisely why
the table above puts the socket layer in `Sys` and everything above it
outside.
...
| **P2** | Memory model (§4.1) · ~~`S1` unboxed nullary constructors~~ **(DONE)** · ~~`Map` throughput~~ **(DONE, 1.80×)** | ~~`Map` insert at 10⁶ within 2× of Rust~~ ✓ met — and not by allocator work: the cost was an affine hash, corrected in B3. §4.1's actual subject — deterministic reclamation for long-running programs — is **met for the LSP**, which is the case the roadmap named and the one that had a number: an editing session held 3.4 MB per edit and 693.7 MB after two hundred, and is now flat at 6.8 MB, gated by `check-lsp-selfhost.sh` with the boundary-removed ablation run. Three things got it there: the allocator returns chunks a reset passes rather than stranding them (576 KiB per iteration, measured); reclaimed memory is handed back zeroed, which it was not, and which `Map`, `Intern` and `Str` all assume; and `__axiom_arena_reset_keeping` makes reclaim-and-copy one operation, because as two it scrubs the source whenever the live set exceeds the garbage. **The strategy for what remains of P2 is decided and most of it is BUILT** ([docs/memory-model.md](memory-model.md) MM-LIFE-2a–2f): automatic reclamation is reference counting — not the compiler inserting these arena calls, which is withdrawn (MM-ALLOC-17–19). The blocker this row named — the compiler cannot see pointerhood (§2.2, MM-ALLOC-20) — is **gone as of 2026-08-15**: the `String`/`Int` fiat was deleted after a campaign took the fiat-off residual from 1,593 to 0 and the corpus differential to 0 of 297 files, and a type VARIABLE's pointerhood is answered at run time by the evidence word. Landed the same day, each behind the full gate battery: the count/shape header on every block (2b), the release path and size-class reuse (2e mechanism), the reference maps in both halves — monomorphic from declared field types and polymorphic from evidence (2d, cliffs `AX3029`/`AX3030`), the `Str` owner word so a slice keeps its parent's bytes by arithmetic (MM-VAL-7, MM-LIFE-6 half discharged), `match` result cells off the heap entirely (MM-ALLOC-9 amended), and the first ownership events — births, field retains, the `handle` record's release — so a program reclaims memory with **no reclamation in its source** (`tests/stdlib/355-arc-events.ax`). Four more rungs landed 2026-08-15, each gated: the `Str` header's own map, so a dead string frees its BYTES and not just its header (a thousand build-and-drop iterations move the bump 384 bytes where they moved it 80,304 — `tests/stdlib/359-arc-str-bytes.ax`); the evidence record's map and its two retains, so a handler built at the handle dies with it (`360-arc-evidence-map.ax`); event 5, a field store retaining the new value and releasing the old, which went first of the events because its balance is LOCAL — provable without escape analysis (`361-arc-field-store.ax`); and `Sys.sysReadAll`'s missing owner share, a use-after-free that was inert only because nothing released a `Str` header yet (`358-str-owner-shares.ax`). **The blocker was solved 2026-08-15 and event 4 landed on top of it.** The two invisible stores - `Mem.memSetWord` and the AST's `mkNode`, each of which `cast Int`s a value out of the type system's sight - now take a share through `__retainref`, a type-directed retain that emits nothing at all for an integer argument (MM-LIFE-2g). With the stashes visible, the self tail call became a release boundary: a loop allocating a fresh string per iteration moves the bump **480 bytes over 2000 iterations** where it moved **224,304** (`tests/stdlib/362-arc-tail-boundary.ax`, 63). The fixture asserts both halves, because with the event and without the share the loop is still flat and a `Vec` element pushed 300 boundaries earlier reads a length of 2 - freed and re-issued. Events 2 and 3 remain, and they are the two that need an ESCAPE analysis rather than a matched pair over one slot. **What the measurements below still say is unchanged, and it is a different fact:** Events 2, 3 and 4 need the values they release to be visible to counting, and this compiler's are not: a `String` pushed into a `Vec` or interned reads count 0 afterwards, and the same string stored into a struct field reads 0 through an `Int`-declared field and 1 through a `String`-declared one — the declared type is the entire difference. `ASTNode` declares all ten of its fields `Int`. So does the Life probe's board, which is why §4.1's own acceptance measurement cannot move: `advance` is `(-> Int Int Int)` and a type-directed event can never fire on it. The LSP half was run rather than quoted: with the arena boundary removed the 200-edit session grows 193,163 bytes per edit against 840 with it — the flatness is the boundary's doing, entirely, and §3.3's refusal would therefore be a regression if it shipped on schedule. That conclusion was superseded the same day: the declared type is thrown away in exactly TWO places, both a `cast Int` inside a polymorphic function, and `__retainref` closes them (MM-LIFE-2g) — so the next slice was never the typing campaign. What the acceptance measurements still wait on is events 2 and 3, which need an escape analysis — see [docs/memory-model.md](memory-model.md) MM-LIFE-2c and MM-LIFE-2e, which carry the probes. The single-shot compiler process has no P2-blocking measurement, and the long-running server that would have carried one is not a v1 subject any more — HTTP is out of scope (§4.5), so the LSP is now the only long-lived Axiom program v1 ships, and it is the one this row measures |
| **P3** | Macro system (§4.2) — expansion moved before the checker, binder hygiene, coverage and refusals done (2026-08-09); module-qualified macros done (2026-08-14: `Pre::when` expands, private ones are `AX3023`, and an entry-file function outranks an imported macro of the same name — `tests/selfhost/368-macro-qualified.ax`, `369-macro-vs-function.ax`); declaration macros v1 done (2026-08-14: rule-form macros generate `fn`/`::` declarations and further invocations from the entry file, phase D fixpoint before phase E, `AX3027` for every refusal — `tests/selfhost/372-decl-macro.ax` 144, `373-decl-macro-types.ax` 10); `derive` ships and the `syntax/*` query table CLOSED on 2026-08-15 — `syntax/name`, `syntax/arity` and `syntax/defined` landed with the prelude macros that spend them (`deriveShow`, `deriveArity`, `showOr`; `tests/selfhost/380-syntax-scalar-queries.ax`, 41), and a join now stands where a reference stands so a macro can CALL what it names. `data` and `struct` joined the template surface the same day, which is what makes MAC-CAP-7 hold: a macro invents a type and `deriveEq` derives over it in the same phase-D round (`381-macro-type-templates.ax`, 32). Module-side invocation landed 2026-08-15 - a module invokes a declaration macro over its own declarations and phase D applies the mangling and visibility import resolution would have, so a library can spend `stdlib/Pre.ax`'s `deriveEq` on its own types (`tests/selfhost/388-module-side-decl-macro.ax`, 239). Multi-rule macros landed 2026-08-15 and rules stopped being selected by counting the next day: a rule's parameters are PATTERNS and selection is a match in rule order, arity surviving inside it as a pre-filter (MAC-LANG-15/18; `AX3033` for a rule an earlier one starves — `tests/selfhost/390-multi-rule-macro.ax` 51, `392-macro-patterns.ax` 127). Repetition followed the same day (MAC-LANG-16 v1: a rule's last element may repeat, which is the variadic macro the language did not have; `AX3034` for a depth error — `393-macro-ellipsis.ax` 63). This sentence read "Repetition patterns remain; MAC-LANG-14's own multi-rule example does not parse today (AX2003 at the second rule)" until 2026-08-16 — that example parses and runs now, and the spec dates the measurement rather than deleting it. What MAC-LANG-14 still specifies and does not hold is rules over EXPRESSION templates; what MAC-LANG-16 v1 does not do is repeat over a nested pattern; and MAC-LANG-17's literal identifiers are still behind scope sets. **The macro system's first user-facing payoff landed 2026-08-15: printing is now two macros and no per-type functions** (MAC-CAP-10). This cell said FIVE until 2026-08-16, counting `print` and `eprint`, which were never written - `stdlib/IO.ax` says outright that there is deliberately no newline-less printer, and `(print "hi")` is `AX3001`. `syntax/format`/`syntax/formatln` parse a format string at expansion time, so `(println "n={n:>8x}")` is a rewrite into `fmtPadLeft`/`fmtHex`/`show` calls and no format string survives to run time; `println` and `eprintln` are one line each on top of them, with `format` beside them in `Show` for the string a program builds instead of writing. The per-type family is GONE - `IO` exported `printInt`/`printlnInt` until that day, and their 445 call sites moved to `println`, which reaches the same rendering through the `Show` trait. Taking the name `println` for a macro cost **zero** call sites, because a non-literal argument lowers to one `show` and `Show`'s `String` instance is the identity. It also made `(println "hi")` cheaper than the function it replaced: the newline folds into the literal at expansion time, so it is one `write` of one static constant (`c"\68\69\0A\00"`) where the function did two writes. Gated by `tests/selfhost/382-format-macros.ax` (255, eight independent claims one bit each), `tests/stdlib/365-format.ax` against a golden stdout, and `tests/diagnostics/570-format-refusals.ax` for all nine `AX3031` cases. Reaching for it found three defects that predate it, each now fixed and each of the class this document keeps a list of: trait dispatch **emitted a call to a function that does not exist** when the argument had no named type (`call i64 @show`, rejected by `opt` as AX4003 against `<toolchain>`); every diagnostic inside a dispatch argument was **doubled**, because selecting an implementation checks that argument and the ordinary walk checks it again; and the REPL **never expanded imported macros at all** - `(when true 5)` was `undefined variable `when`` at the prompt, bug-for-bug parity with a stage0 that no longer exists · ~~`B4` namespacing~~ **(DONE)** | Hygiene test passes (`tests/selfhost/361-macro-hygiene.ax`, 143 against the unfixed compiler's 208); two modules define the same name without collision. A bare reference *inside* a module now binds to that module's own definition first, in both compilers: before it, an entry file defining `helper` silently redirected an imported module's own call to its own `helper` — importing a module changed what that module did, with no diagnostic from either side (`tests/selfhost/850-module-local-binding.ax`). Resolution outward from a module is still the merged declaration list rather than that module's import set, which is the remaining half of the flat-namespace decision. **Module visibility is now real** (2026-08-10): `pub` and an import's name list decide which names are visible, not which declarations exist, so a module keeps its private helpers and behaves identically however it is imported. The same conflation that `850` fixed for a *defined* name was still live for a *dropped* one — `(import IO (println))` beside an entry-file `writeStr` made the standard library print the entry file's bytes at exit 0 — and reaching a name a module does not export is now `AX3023` rather than a link failure. 246 corpus files, 0 divergences; `tests/selfhost/920-private-declaration.ax`, `930-selective-import.ax`, [self-hosting.md §14](self-hosting.md). An import's name list is checked (`checkImportNames`; this cell said "still unchecked" long after it stopped being true) |
| **P4** | Self-hosting phases 2–5 | `stage2 == stage3` ✓. (This row carried "HTTP server serves a request under load" until 2026-08-15; HTTP is out of scope — §4.5 — and the criterion went with it rather than being marked unmet.) Phase 3 (semantic analysis in Axiom) has begun and meets its criterion for the checks that exist: stage1 emits **byte-identical AXDL** for the **thirty-two** `AX30xx` codes the corpus provokes, pinned against a checked-in golden by `scripts/check-diagnostics.sh` over **126 cases** (96 `.ax`, 30 `.axbad`). This cell read "all seventeen … gated three-way … (36 cases)" until 2026-08-16 and every part of it had moved. The count: the compiler defines 33 `AX30xx` codes today (`AX3001`–`AX3034` less the retired `AX3032`), and the goldens carry 32 of them — `AX3022`, the macro `set`-target refusal, has a construction site and no fixture, which is the one gap this re-derivation found. The gating: the third arm is not repointed but GONE, because a differential does not fail when its reference disappears — aiming stage0's variable at a self-hosted binary compares a compiler with itself and reports zero differences, so the gate's own header records the removal and what replaced it: the checked-in golden plus `tests/diagnostics/verify-axdl-spans.py`, which re-derives every span against the fixture's own bytes and cannot be satisfied by re-blessing. Type checking proper is done — `parseSigDecl` keeps type structure, and `AX3004`/`AX3005`/`AX3007`/`AX3008` compare types exactly where stage0 does, with no unifier. Effect inference is a monotone fixpoint over the call graph with effect-transparent parameter marks, so its sets match stage0's. Imported modules are checked too, each rendered against its own filename. Phase 3's exit criterion is met; grouping is a no-op on both sides, since no producer in stage0 ever sets a group key. Phase 4 re-measured (2026-08-07): 0 of 71 comparable `.ll` pairs are byte-identical, and the raw-versus-post-`mem2reg` question the criterion was waiting on is now answered — normalising does not help (46 diff lines becomes 50 on the simplest program). What survives is stage1's calling convention, block structure and register naming, all three deliberate; the arithmetic already matches. So the criterion asked stage1 to adopt the retiring compiler's design choices rather than to converge on lowering, and **it has been replaced**: phase 4 now exits on behavioural equivalence between the compilers over both corpora at both optimisation levels through an identical toolchain, plus the four-target assembly check and the fixpoint (which still requires byte-identity where it carries meaning, `stage2 == stage3`). Widening the differential gate to carry that criterion found stage0's own output overflowing the stack on `320-effect-gc-roots` through a raw `llc` pipeline where stage1's does not, because stage0's frames carry a hidden closure parameter and spill their bindings — converging stage1's IR onto stage0's would have imported that. It also found the gate **was not in CI at all**, which is the risk this table's last-but-four row names; it runs there now, over 106 cases with a counted floor. Scouting also found that **nothing ran `tests/stdlib/` through stage1**, hiding five miscompiles; `scripts/check-stdlib-selfhost.sh` now gates them, and that gate still has **no skip list at all** — **56 cases** today, where this cell said "33 of 33 agree" from 2026-08-07 and the corpus has since grown by 23 — because the effect system lowers through stage1 too (evidence slots, outward dispatch from a handler performing its own effect, and the unhandled-operation trap). Doing it exposed a scalability ceiling underneath: the lexer's `lexTokens`/`dispatchChar` mutual tail calls cost a stage1-built compiler one frame pair per token, so it died between 96 KB and 146 KB of input where a stage0-built one read 396 KB. Both are loops now; measured past 814 KB |
| **P5** | Driver, bootstrap, fixpoint · LSP (§4.6) · ~~trivia preservation for `fmt`~~ **(DONE, §2.3)** · benchmarking · docs. Phase 5's first item — module resolution — is **done**: stage1 reproduces stage0's search order (entry dir, `AXIOM_PATH`, `AXIOM_STDLIB`, exe-relative), where it previously searched two paths relative to its working directory and could not compile a file importing `IO` from anywhere else. Its host target is a per-target `Host` module rather than the literal `"darwin-aarch64"`, which is what the Linux fixpoint job needed to mean anything. `Sys` can now spawn and wait for child processes on all four targets, which is the capability a self-hosted `build` needs. **The driver is done**: `stage1 build --input self_host/main.ax --output stage2` writes the IR, runs `opt`/`llc`/`cc` and cleans up after itself, and the compiler it produces builds a byte-identical successor — `check-bootstrap.sh` runs the ladder a second time with the compiler driving the toolchain instead of the harness, and finishes by building and running a real program through the self-built compiler. `check`, `emit-llvm`, `run` and `version` are the rest of the surface; the old `stage1 FILE [TARGET]` spelling still works, because five harnesses use it. ~~What remains for P5 is `fmt`~~ — **`fmt` is done and gated** (2026-08-07): `stage1 fmt` formats all 194 repository files to byte-identical output with identical exit status under both compilers, `--check` included; the zoo is three-way against the checked-in golden; and a 33-case parity bank pins the deliberate refusals. `scripts/check-fmt-selfhost.sh` is the gate, in CI, floors counted, negative test run (an ablated normalisation fails it). The design held as decided — a concrete syntax tree with its own scanner, thirteen normalisations as rewrites — and the port surfaced stage0 behaviours nobody had documented, now reproduced bug-for-bug or refused exactly as stage0 refuses; see [self-hosting.md](self-hosting.md#the-formatters-back-half-the-same-function-measured). ~~`symbols` and `explain`~~ are **done and gated** (2026-08-07, same day): `explain` generated byte-for-byte from stage0's own output, 40/40 on its first differential; `symbols` at 196/196 files with identical bytes and status in both modes, after moving stage0's nid to a specified hash (FNV-1a 64 - `DefaultHasher` is unspecified across toolchains) and fixing what the port dislodged: stage1's import order now hoists like stage0's, imported modules keep their AXTAGs, `type`/`trait`/`deriving`/multi-body-`fn` parse, and `[` `]` are tokens instead of silently skipped bytes. `scripts/check-tools-selfhost.sh` gates both, live and two-sided, floors counted, negative tests run. ~~The human diagnostic renderer~~ is **done and gated** (2026-08-07): `stage1 check` renders rustc-flavored plain-text reports by default — snippets, char-counted caret columns, every help the diagnostic carries — plus stage0's JSON shape and the exact failure trailer, behind `--diagnostic-format` with stage0's full alias set. Deliberately NOT a byte-clone of stage0's human output: that output is the `ariadne` crate's (probed: per-character ANSI even when piped, and all helps but the last silently dropped), an internal of the retiring compiler — the phase-4 precedent applied to a tool. `scripts/check-render-selfhost.sh` gates it in CI: against 126 checked-in `.human` goldens, one per `.axdl` with no skip list (36 when the renderer landed; this figure read 56 until 2026-08-16, one of three counts in this row left behind by a growing corpus), cross-checked fact-by-fact against the AXDL goldens stage0 itself equals, exit statuses compared to stage0's per case, floors counted, caret-ablation run (36/36 fail ablated, 36/36 pass restored). The port surfaced two pre-existing divergences the AXDL gate could not see: warnings-only files exited 1 under stage1 where stage0 exits 0 (fixed — only errors gate), and stage0 prints the failure trailer in `ai` mode where stage1 printed none (fixed — every format). ~~The REPL~~ is **done and gated** (2026-08-07): `stage1 repl` reproduces stage0's piped surface byte-for-byte over a 12-session bank — every scalar result type, declarations and state, semantic errors and recovery, the full colon-command dispatch including stage0's unreachable-`?` bug, comment/blank handling, `:quit` semantics — 9 sessions byte-identical three-way (golden == stage0 == stage1), 3 marker-pinned where stage0 is nondeterministic or divergent by decision (`:time` brackets the eval with the new `sysNowMicros` - the clock primitive P5's benchmarking item wanted, landed early because a probe of its syscall found the flags-clobber miscompile; `:llvm` prints each compiler's own IR; `:defs` and stage0's leaked `_fn_0` tyvar under redefinition are recorded, not cloned). Interactive affordances (rustyline editing, history) are deliberately not ported — the piped surface is the tested contract and the editor-grade interface is the LSP's business. Porting it found and fixed a latent lexer bug: a string whose closing quote is the input's last byte lexed as `TK_ERROR`, impossible to hit from a module file and the first thing the REPL fed the lexer. `scripts/check-repl-selfhost.sh` gates it in CI, floors counted, stderr required empty, ablation run (9 sessions fail without the `type :` line; 12/12 restored). **The P5 tool-port list is empty**: every stage0 tool surface — check, build, run, emit-llvm, fmt, symbols, explain, the human renderer, the REPL — now exists in the self-hosted compiler, gated two-sided. What remains of P5 is benchmarking (its clock primitive already landed) and docs. **The LSP's first slice has landed** (2026-08-08): `axiom lsp` speaks the lifecycle, full-text sync, `publishDiagnostics` and `documentSymbol` over JSON-RPC on stdin/stdout, reusing the self-hosted frontend rather than reimplementing it — 1,400 lines across `self_host/lsp.ax`, `stdlib/Json.ax` and `stdlib/Rpc.ax`. It is the first tool surface with no stage0 counterpart to differ against, so `scripts/check-lsp-selfhost.sh` is the native-surface shape the human renderer established: 7 fixtures (floor 7) byte-gated against checked-in goldens for the whole framed session, PLUS every published diagnostic compared fact-by-fact against stage0's own AXDL — severity, code, line, and the column converted from stage0's 1-based character index into LSP's 0-based UTF-16 code unit. Running the mandatory vacuousness drill caught the gate: with `lspChar` ablated to a byte count and every golden RE-BLESSED from the ablated build, the first version passed 7/7, because it compared lines and not columns. It now fails that ablation (server 23, stage0 20) and passes restored. The server built by stage0 and the server built by stage1 emit byte-identical protocol streams; peak RSS for one self-compile moves 176.8 → 187.0 MiB against a 400 MiB ceiling. Hover and go-to-definition arrived on 2026-08-15, for MACRO INVOCATIONS only: both are answered off the raw parse tree — the macro's own declaration span, and its declaration rendered verbatim — because a macro invocation is the one position whose meaning needs no type. This sentence said all three were NOT in it until 2026-08-16. Over an ordinary expression they still are not, and completion is not implemented at all: those need a type at a position, and `typecheck.ax` keeps no node-to-type table. That table is one prerequisite behind two rungs — LSP hover over an expression, and the memory model's ownership events 2 and 3, which need the type of a `let` binding for the same reason (see [memory-model.md](memory-model.md) MM-LIFE-2c) | Completion and diagnostics in a real editor; ~~`fmt` round-trips every file in the repo, gated in CI~~ ✓ 390/390, gated by `check-fmt.sh`; published performance profile |

**On effort.** P1 is weeks. P2's research question is answered —
reference counting ([memory-model.md](memory-model.md) MM-LIFE-2a) — and
its remaining effort is implementation behind the pointerhood
prerequisite (MM-ALLOC-20), not option-picking in §4.1. P3–P5 are
