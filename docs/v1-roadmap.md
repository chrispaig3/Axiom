...
| A macro that introduces a binding named `x` does not capture a user's `x`, and the reverse | **Half, and the halves are not symmetric.** The forward direction is done for every binder a template can introduce — `let`, `let mut`, `lambda`, `match` arm patterns — by renaming to `<name>.<counter>`, gated by `tests/selfhost/361-macro-hygiene.ax` (143; the unfixed compiler answers 208). The reverse needs definition-site resolution, which needs macros to be module-qualified, which they are not — and that is a live wrong-answer bug, not just a missing feature: one macro in one module means two different things depending on the caller's module (measured, 6 against 105). See [macros.md §3](macros.md) |
