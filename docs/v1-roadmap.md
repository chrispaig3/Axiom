...
| **P4** | Self-hosting phases 2–5 · HTTP library (§4.5) | `stage2 == stage3` ✓; HTTP server serves a request under load. Phase 3 (semantic analysis in Axiom) has begun and meets its criterion for the checks that exist: stage1 emits **byte-identical AXDL** for twelve of the seventeen `AX30xx` codes, gated three-way against a checked-in golden by `scripts/check-diagnostics.sh` (29 cases). Type checking proper is done — `parseSigDecl` keeps type structure, and `AX3004`/`AX3005`/`AX3007`/`AX3008` compare types exactly where stage0 does, with no unifier, because a checker that knows more than stage0 reports what stage0 does not. What remains of phase 3: the effects/AXTAG cluster (`AX3010`/`AX3011`/`AX3016`/`AX3017`), `AX3014` ambiguous-name, diagnostic grouping, and checking imported modules' bodies |
