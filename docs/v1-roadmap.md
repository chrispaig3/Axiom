...
| | Status |
|---|---|
| A `derive`-style macro generates a working `Eq` instance for a `data` type, with exhaustiveness still checked on the generated `match` | **Closer than half (2026-08-14).** Exhaustiveness *is* checked on a generated `match` — the same `AX3005` the hand-written one draws — and the mechanism `derive` needs now exists: a rule-form macro in declaration position generates `fn` and `::` declarations, checked and compiled like hand-written ones (`tests/selfhost/372-decl-macro.ax`, 144; the unfixed compiler exits 1 on a bare `AX2003`). What keeps this cell open: `deriveEq` must *inspect* the `data` type's constructors, and the `syntax/*` query vocabulary (MAC-CAP-5/6) does not exist; `deriving (Eq)` still parses and is discarded, with refusal the settled replacement (MAC-CAP-9) |
