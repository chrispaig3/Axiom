...
**Half of that is now in place** (2026-08-09). `Diag` carries a `trace`
vector, AXDL renders it as `&"frame"` per entry, the human renderer as
`= note: in this expansion of ...`, the JSON as an `expansion` array,
and the LSP appends it to the message. Nothing populates it yet, because
nothing should until expansion reports through diagnostics — the field is
ready and `tests/selfhost/645-axdl-repetition` is what proves it renders.
What the macro work still owns is threading a span's provenance through
expansion and filling the vector in.
...
| Macros degrade diagnostics | Expansion backtrace in `Diag` is part of the macro work, not a follow-up. The FIELD exists and renders in all four surfaces as of 2026-08-09; populating it is the macro slice's job |
