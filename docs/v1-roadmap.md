...
| **P4** | Self-hosting phases 2–5 · HTTP library (§4.5) | `stage2 == stage3` ✓; HTTP server serves a request under load. Phase 3 (semantic analysis in Axiom) has begun and meets its criterion for the checks that exist: stage1 emits **byte-identical AXDL** for sixteen of the seventeen `AX30xx` codes, gated three-way against a checked-in golden by `scripts/check-diagnostics.sh` (33 cases). Type checking proper is done — `parseSigDecl` keeps type structure, and `AX3004`/`AX3005`/`AX3007`/`AX3008` compare types exactly where stage0 does, with no unifier. Effect inference is a monotone fixpoint over the call graph, without effect polymorphism, so its sets are a sound subset of stage0's. What remains of phase 3: `AX3014` ambiguous-name, effect polymorphism (without which `AX3010` stays gated off higher-order functions), diagnostic grouping, and checking imported modules' bodies |
