...
| **P5** | Driver, bootstrap, fixpoint · LSP (§4.6) · ~~trivia preservation for `fmt`~~ **(DONE, §2.3)** · benchmarking · docs. Phase 5's first item — module resolution — is **done**: stage1 reproduces stage0's search order (entry dir, `AXIOM_PATH`, `AXIOM_STDLIB`, exe-relative), where it previously searched two paths relative to its working directory and could not compile a file importing `IO` from anywhere else. Its host target is a per-target `Host` module rather than the literal `"darwin-aarch64"`, which is what the Linux fixpoint job needed to mean anything. `Sys` can now spawn and wait for child processes on all four targets, which is the capability a self-hosted `build` needs. **The driver is done**: `stage1 build --input self_host/main.ax --output stage2` writes the IR, runs `opt`/`llc`/`cc` and cleans up after itself, and the compiler it produces builds a byte-identical successor — `check-bootstrap.sh` runs the ladder a second time with the compiler driving the toolchain instead of the harness, and finishes by building and running a real program through the self-built compiler. `check`, `emit-llvm`, `run` and `version` are the rest of the surface; the old `stage1 FILE [TARGET]` spelling still works, because five harnesses use it. What remains for P5 is `fmt`, `symbols`, `explain`, the REPL, and the human diagnostic renderer. `fmt` is the one any gate depends on — its five call sites are all inside `check-fmt.sh`, which is the single gate that cannot run with `$AXIOM` pointed at a stage2. Its design is decided and measured rather than open: stage1's `fmt` formats from a **concrete** syntax tree with its own scanner, not from the compiler's AST and not from the compiler's lexer. Both halves of that were forced by measurement. stage1's lexer fuses `-` against a following digit into one token where stage0 emits two, so a formatter sharing it could not reproduce stage0's output whatever else it did; and stage1's parser is deliberately lossier than stage0's — it consumes `type`, `trait` and `impl` into an inert node and desugars `(:: e T)` and `(- x)` — so an AST-driven port would delete declarations and misprint negation. The printer owes thirteen measured normalisations, each of which is a rewrite rather than a reprint: `define`→`fn`, `(fn f = b)`→`(fn (f) b)`, `1_000_000`→`1000000`, `007`→`7`, `((g 1) 2)`→`(g 1 2)`, `(e)`→`e`, `{e}`→`e`, a multi-expression body gaining a `{}`, `(-> a (-> b c))`→`(-> a b c)`, `(T)`→`T`, `()`→`Unit`, `Integer`→`Int`, and `X.Y::z`→`X::Y::z`. Its exit criterion is symmetric: stage0 and stage1 `fmt` are the same function, on identical bytes **and** identical exit status, three-way against a checked-in golden | Completion and diagnostics in a real editor; ~~`fmt` round-trips every file in the repo, gated in CI~~ ✓ 190/190, gated by `check-fmt.sh`; published performance profile |
