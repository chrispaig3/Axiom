...
| ~~`AX3041`~~ | ~~`foreign-primitive`~~ | — | **not needed, and never allocated** | The condition is already refused as `AX3004`. Every arena primitive and every raw load/store takes `Int`, and `tyCompat` requires named constructors to match BY NAME, so `(memGetWord h 0)` on a `Foreign` is a type mismatch before any FFI-specific rule runs. Measured, and pinned by `tests/diagnostics/720-foreign-arena-primitive.axbad`. A code is never reused in this repository, so one is not spent on a refusal that already exists |
| `AX4004` | `ungrounded-symbol` | error | `self_host/driver.ax`, before `opt` | an `extern` item names a symbol no archive on the link line defines. A **build** diagnostic: its condition names the link line, which `check` does not have |
...
> **Which of these exist today: all of them.** Twenty-two fixtures are
> written and passing — `tests/ffi/no-extern/` (3), `tests/ffi/nostd/`
> (1), `tests/ffi/demo/` (14, including 400-arc-retain.ax,
> 410-foreign-not-walked.ax and 420-null-foreign.ax),
> `tests/ffi/probe-leaky/` (1) and `tests/ffi/probe-ungrounded/` (1) —
> plus `tests/diagnostics/720-foreign-arena-primitive.axbad`. Nothing in
> this section names a file that does not exist, which is what
> `scripts/check-doc-drift.sh` enforces.
...
**`400-arc-retain.ax` — does a crossing leak?** Four blocks, each 20,000 crossings:
...
**`720-foreign-arena-primitive.axbad`** — an arena primitive applied to a `Foreign` must be **refused at check time**, and it is: as `AX3004`, not as the new `AX3043` this paragraph once predicted. Every arena primitive takes `Int`, and `tyCompat` matches named constructors by name, so the refusal falls out of the type relation with no FFI-specific rule at all. MM-FFI-2 already documents that passing non-arena memory as the kept block is undefined; MM-FFI-5(2) upgrades it from documented-undefined to refused, and this fixture is the upgrade.
